...
/* sierra_pixel: turns on the ordered dither below (present in every style
   pack's copy of this shader, but only called here) to break up the
   palette LUT's quantization bands into a King's Quest-style stipple.
   classic's copy of this file does not declare this uniform at all, so
   asset_loading.inc's SetShaderValue call for it is a harmless no-op
   there. 0 keeps the shader byte-for-byte behaviorally identical to
   classic. */
...
/* Centered -0.5..0.5 so it can be added straight onto a color. gl_FragCoord
   is in the low-res render target's own pixel space (the 3D scene draws
   directly into that target, and the camera is snapped to its pixel grid
   by SnapCameraToArtPixels), i.e. art-texel space -- not the upscaled
   window -- so the pattern holds still on screen instead of shimmering as
   the camera moves. */
float artDitherOffset()
{
    return orderedDither4x4(gl_FragCoord.xy) - 0.5;
}
...
void main()
{
    ...
    if (isTerrain > 0.5 && horizonFog > 0.5) {
        ...
        vec3 dithered = mix(clamp(paint, 0.0, 1.0), fogColor, haze) +
            artDitherOffset() * ditherStrength;
        finalColor = vec4(clamp(dithered, 0.0, 1.0), albedo.a);
        return;
    }
    ...
    color += artDitherOffset() * ditherStrength;
