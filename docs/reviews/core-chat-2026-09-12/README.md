...
Validation uses the full native test suite and static analysis with the existing reviewed baseline. Run `ctest --test-dir out/build/native-core --output-on-failure` and `python3 tools/static_analysis.py` from the repository root. The browser build packages the 5,044,766-byte model separately; its startup data pack is 5,872,138 bytes, within the existing 10 MiB limit. The browser fetches the model before play, then generates locally.
