...
Recorded on clean Release commit `12fcf0951e8e5f1ee69a8b80fa631722f3efa9dd`: four policy/site cases with two matching runs each and 41 checkpoints per run. All site stock equations and repair cost/work/gain equations pass at every checkpoint. The headless suite passed all 82 tests; the final focused preview checks also passed. All 880 historical annual hashes matched the parent across the schemas in `parity.json`. The manifest and outcomes pin the capture provenance and final site totals.
