...
Recorded on clean Release commit `5775f2098ba38e43f4480d551f67d444da32cdc1`: four policy/site cases with two matching runs each and 41 checkpoints per run. All site stock equations and repair cost/work/gain equations pass at every checkpoint. The headless suite passed all 82 tests; the final focused preview checks also passed. All 880 historical annual hashes matched the parent across the schemas in `parity.json`. The manifest and outcomes pin the capture provenance and final site totals.

The Linux compiler check caught an event that could exceed the text capacity. The shortened message retains the actual repair cost and final condition. The repeated capture on the corrected commit preserves all reported values apart from state hashes, which include event text.
