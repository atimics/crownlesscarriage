...
raw=json.loads((root/'hash-runs.json').read_text())
expected={r['ordinal']:r['hashes'] for r in raw if r['schema']==87}
legacy={r['ordinal']:r['hashes'] for r in raw if r['schema']==86}
reference=json.loads((root.parent/'archive-volume-rules-2026-09-08/parity.json').read_text())
for row in reference['worlds']:assert legacy[row['ordinal']]==row['hashes']
assert len(rows)==32
assert {(r['arm'],r['ordinal']) for r in rows}=={(arm,n) for arm in ('protected',) for n in range(1,33)}
matched=0
for row in rows:
...
assert matched==3200
report={'runs':len(rows),'failures':sum(r['exit_code']!=0 for r in rows),'observer_parity_annual_hashes':matched,'legacy_parity_annual_hashes':3200,'trace_hashes_verified':len(rows),'cargo_accounting':'ordered = delivered + redirected + lost + unresolved'}
(root/'verification.json').write_text(json.dumps(report,indent=2)+'\n')
print(json.dumps(report,indent=2))

parent={r['ordinal']:r for r in json.loads((root.parent/'archive-funding-controlled-2026-09-08/results.json').read_text()) if r['arm']=='protected'}
paired={}
for key in ('zero_staff','ready','staff_sum','lore_sum','end_lore','spent'):
    deltas=[r[key]-parent[r['ordinal']][key] for r in rows]
    paired[key]={'higher':sum(d>0 for d in deltas),'equal':sum(d==0 for d in deltas),'lower':sum(d<0 for d in deltas),'total_delta':sum(deltas)}
