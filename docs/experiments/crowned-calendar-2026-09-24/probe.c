/* Frozen measurements for the 364-day Crown Age comparison. */
...
static int seen_crowns, seen_editions;
...
static void Daily(int seed, History *species) {
    ...
    for (; seen_crowns < s.crown_calendar.sighting_count; ++seen_crowns) {
        const CcScrivenNote *n=&s.crown_calendar.sightings[seen_crowns];
        EventLine(year,"crown-sighting",n->day,n->text);
    }
    for (; seen_editions < s.crown_calendar.editions; ++seen_editions) {
        const CcScrivenFinding *f=&s.crown_calendar.almanacs[seen_editions];
        char text[192]; snprintf(text,sizeof(text),"Crown Age begins day %d; agreed day %d by %d schools; books %llu and %llu",
            f->proposed_day,f->agreed_day,f->schools,(unsigned long long)f->book_ids[0],(unsigned long long)f->book_ids[1]);
        EventLine(year,"crown-almanac",f->agreed_day,text);
    }
    (void)seed;
}
...
#undef X
    fprintf(extra,",%llu,%llu,%d,%d\n",(unsigned long long)empty_arrivals,
        (unsigned long long)loaded_arrivals,s.crown_calendar.sighting_count,s.crown_calendar.editions);
}
