...
static void window(const CcSim *initial,const char *label,int treatment) {
    ...
    for(int d=0;d<36500;++d){
        ...
        bool heard=false;for(int i=0;i<CC_MAX_GOSSIP;++i)if(s->gossip[i].heard_day==s->current_day)heard=true;heard_days+=heard;
...
int main(int argc,char **argv){
    ...
    if(argc!=2 || !CcSaveRead(argv[1],s,error,sizeof(error)))return 2;
