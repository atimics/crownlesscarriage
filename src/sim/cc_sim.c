...
CcMoney CcSimTrackedGold(const CcSim *sim)
{
    ...
    if (sim->schema_version >= 89U) total += sim->archive_convoy.purse;
    for (int32_t i = 0; i < sim->kingdom_count; ++i) {
...
int32_t CcSimTrackedGood(const CcSim *sim, CcGood good)
{
    ...
    if (sim->schema_version >= 89U && good == CC_GOOD_WHEAT) total += sim->archive_convoy.wheat;
...
static bool BindArchiveTome(CcSim *sim)
{
    ...
    return BindArchiveTomeAt(sim, vault, sim->schema_version >= 84U && sim->archive_staff.active) != NULL;
}
...
static bool AppointArchiveRecruitReady(CcSim *sim, CcArchiveAppointmentPlan plan)
{
    ...
    if (plan.account_slot >= 0) volume = BindArchiveTomeAt(sim, seat, sim->schema_version >= 84U);
...
static void AdvanceRoyalCarriages(CcSim *sim, CcRoadProductionAccounting *site_accounting)
{
    ...
    for (int32_t i = 0; i < sim->royal_carriage_count; ++i) {
        ...
        if (IsSiteCarriage(carriage) ||
            (sim->schema_version >= 89U && carriage->mode >= CC_ROYAL_CARRIAGE_ARCHIVE_RESERVED)) continue;
        const CcSettlement *location = CcSimSettlement(
            sim, carriage->location_id);
    ...
    if (sim->schema_version >= 38U) {
        for (int32_t i = 0; i < sim->royal_carriage_count; ++i) {
            ...
            if (CcIdKind(carriage->id) != CC_ENTITY_ROYAL_CARRIAGE ||
                ...
                location == NULL ||
                carriage->mode < CC_ROYAL_CARRIAGE_IDLE ||
                carriage->mode > (sim->schema_version >= 90U ? CC_ROYAL_CARRIAGE_ARCHIVE_WAITING : sim->schema_version >= 89U ? CC_ROYAL_CARRIAGE_ARCHIVE_RESERVED : CC_ROYAL_CARRIAGE_WAITING_CAPACITY) ||
