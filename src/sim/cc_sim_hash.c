...
#define HASH_VALUE(value) hash = HashU64(hash, (uint64_t)(value))
    if (sim->schema_version >= 117U) HASH_VALUE(CcWantsHash(&sim->wants));
    ...
    if (sim->schema_version >= 120U) HASH_VALUE(sim->player.cargo_overflow_allowance);
