    ...
    for (int32_t i = 0; i < (int32_t)(sizeof(WORLD_BUILDINGS) /
        ...
        if (reveal > 0.05f) {
            ...
            DrawBuildingRevealCap(
                ...
                BuildingWallColor(building->style),
                BuildingRoofColor(building->style, kingdom));
        }
        rlPopMatrix();
    }
                       ...
                       float clock)
{
    ...
    for (int32_t i = 0; i < (int32_t)(sizeof(CASTLE_STRUCTURES) /
        ...
        /* Broad stone courses, arrow slits, and battlements turn the bailey
           into a readable fortification from every camera room. They are
           large enough to survive the art-pixel target and do not change the
           collision footprint. */
        for (int32_t course = 1; course <= 2; ++course) {
            float course_y = structure->height * (float)course / 3.0f;
            DrawBox((Vector3){footprint.x + footprint.width * 0.5f,
                              course_y,
                              footprint.y + footprint.height * 0.5f},
                    (Vector3){footprint.width + 0.12f, 0.11f,
                              footprint.height + 0.12f},
                    ShadeColor(stone, course == 1 ? 0.76f : 1.06f));
        }
        if (i >= 5) {
            int32_t front_bays = footprint.width >= 8.0f ? 3 : 2;
            for (int32_t bay = 0; bay < front_bays; ++bay) {
                float slit_x = footprint.x + footprint.width *
                    (float)(bay + 1) / (float)(front_bays + 1);
                float slit_y = structure->height * 0.58f;
                DrawBox((Vector3){slit_x, slit_y,
                                  footprint.y - 0.052f},
                        (Vector3){0.20f, 0.72f, 0.075f}, WORLD_VOID);
                DrawBox((Vector3){slit_x, slit_y,
                                  footprint.y + footprint.height + 0.052f},
                        (Vector3){0.20f, 0.72f, 0.075f}, WORLD_VOID);
            }
            int32_t side_bays = footprint.height >= 7.0f ? 2 : 1;
            for (int32_t bay = 0; bay < side_bays; ++bay) {
                float slit_z = footprint.y + footprint.height *
                    (float)(bay + 1) / (float)(side_bays + 1);
                float slit_y = structure->height * 0.48f;
                DrawBox((Vector3){footprint.x - 0.052f, slit_y, slit_z},
                        (Vector3){0.075f, 0.72f, 0.20f}, WORLD_VOID);
                DrawBox((Vector3){footprint.x + footprint.width + 0.052f,
                                  slit_y, slit_z},
                        (Vector3){0.075f, 0.72f, 0.20f}, WORLD_VOID);
            }
        }
        bool along_x = footprint.width >= footprint.height;
        float long_side = along_x ? footprint.width : footprint.height;
        int32_t merlon_count = (int32_t)ceilf(long_side / 2.35f);
        if (merlon_count < 2) merlon_count = 2;
        if (merlon_count > 12) merlon_count = 12;
        for (int32_t merlon = 0; merlon < merlon_count; ++merlon) {
            float amount = ((float)merlon + 0.5f) / (float)merlon_count;
            Vector3 merlon_center = {
                along_x ? footprint.x + footprint.width * amount :
                          footprint.x + footprint.width * 0.5f,
                structure->height + 0.46f,
                along_x ? footprint.y + footprint.height * 0.5f :
                          footprint.y + footprint.height * amount,
            };
            Vector3 merlon_size = along_x ?
                (Vector3){fminf(0.86f, long_side / (float)merlon_count * 0.54f),
                          0.70f, footprint.height + 0.22f} :
                (Vector3){footprint.width + 0.22f, 0.70f,
                          fminf(0.86f, long_side /
                                             (float)merlon_count * 0.54f)};
            DrawBox(merlon_center, merlon_size, ShadeColor(stone, 0.90f));
        }
        if (reveal > 0.05f) {
            float active_cut_world = reveal_cut_height +
                                     (1.0f - reveal) * 8.0f;
            DrawBuildingRevealCap(
                footprint.x, footprint.y, footprint.width, footprint.height,
                structure->height, active_cut_world - castle_base,
                stone, i >= 8 ? kingdom : WORLD_STONE_SHADOW);
        }
    }
    SetWorldForegroundReveal(0.0f, reveal_cut_height);
    {
                                       ...
                                       int32_t art_height)
{
    ...
    for (int32_t portal_index = 0; portal_index < count; ++portal_index) {
        ...
        int32_t text_width = CcOverlayMeasureText(label, 10);
        float bubble_width = (float)text_width + 20.0f;
