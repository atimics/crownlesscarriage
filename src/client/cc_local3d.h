...
#define CC_LOCAL_TOWN_GATE_X 96.0f
#define CC_LOCAL_TOWN_GATE_Z 36.0f
    ...
    int32_t day, int32_t minute_subticks)
{
    ...
    if (minutes >= CC_CALENDAR_NIGHT_MINUTE) return CC_LOCAL_ATMOSPHERE_MOONLIT_NIGHT;
...
float CcLocalRoadCarriageRouteLengthInternal(const CcSim *sim);
...
Camera3D CcLocalSiteCamera(const CcSim *sim, const CcLocalAgent *agent,
