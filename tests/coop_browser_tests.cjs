...
async function main() {
  ...
  try {
    ...
    const preview = crew.frameLocator('#avatar-preview');
    await preview.locator('body.avatar-preview').waitFor();
    assert.equal(await preview.locator('#touch-panel').isVisible(), false);
    ...
    const mobileLayout = await game.evaluate(() => ({
      ...
      barBottom: document.querySelector('#company-bar').getBoundingClientRect().bottom,
      gameTop: document.querySelector('main').getBoundingClientRect().top
    }));
    assert.equal(mobileLayout.overflow, false);
    assert(mobileLayout.barBottom <= mobileLayout.gameTop, JSON.stringify(mobileLayout));
