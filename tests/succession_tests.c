...
/* Save round-trip at schema 50, and a save stamped 49 loads with the
...
static void CheckSuccessionSaves(void)
{
    ...
    for (uint32_t version = 49U; version <= CC_SIM_SCHEMA_VERSION; ++version) {
        CcSim sim;
        CcSim restored;
        CcSimInit(&sim, UINT32_C(0x50cc0e5));
        ...
        if (version < 77U) CcSimInitializeOccupations(&sim);
