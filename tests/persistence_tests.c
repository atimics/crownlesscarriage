...
{
    bool legacy = schema >= 2U && schema <= 91U;
    ...
    if (generator == CC_GENERATOR_VERSION) {
        ...
        if (schema >= 32U && schema <= 91U) return true;
    }
...
static void CheckSchema79RecruitmentJournal(void)
{
    ...
    sqlite3_close(database);
    AddLegacyDayJournalSuffix(path, &legacy, &after, 79U, 25U);
    CC_CHECK(CcSaveRead(path, &restored, error, sizeof(error)));
    CC_CHECK(restored.schema_version == CC_SIM_SCHEMA_VERSION);
    restored.schema_version = 79U;
    CC_CHECK(CcSimHash(&restored) == CcSimHash(&after));
    restored.schema_version = CC_SIM_SCHEMA_VERSION;
    CC_CHECK(CcSaveWrite(path, &restored, error, sizeof(error)));
    RemoveDatabase(path);
...
static void CheckSchema81RecruitmentJournal(void)
{
    static CcSim legacy, after, restored;
    char error[256];
    const char *path = "persistence-schema81-recruitment.ccsave";
    RemoveDatabase(path);
    CcSimInit(&legacy, 42U);
    legacy.schema_version = 81U;
    CC_CHECK(CcSimBeginArchiveRecruitment(&legacy));
    CcSimAdvanceDays(&legacy, 6);
    after = legacy;
    CcSimAdvanceDays(&after, 1);
    ...
    CC_CHECK(sqlite3_open(path, &database) == SQLITE_OK);
    sqlite3_close(database);
    AddLegacyDayJournalSuffix(path, &legacy, &after, 81U, 25U);
    CC_CHECK(CcSaveRead(path, &restored, error, sizeof(error)));
    CC_CHECK(restored.schema_version == CC_SIM_SCHEMA_VERSION);
    restored.schema_version = 81U;
    CC_CHECK(CcSimHash(&restored) == CcSimHash(&after));
    restored.schema_version = CC_SIM_SCHEMA_VERSION;
    CC_CHECK(CcSaveWrite(path, &restored, error, sizeof(error)));
    RemoveDatabase(path);
...
static void CheckSchema82RecruitmentJournal(void)
{
    static CcSim legacy, after, restored;
    char error[256];
    const char *path = "persistence-schema82-recruitment.ccsave";
    RemoveDatabase(path);
    CcSimInit(&legacy, 42U);
...
static void CheckSchema83RecruitmentJournal(void)
{
    static CcSim legacy, after, restored;
    char error[256];
    const char *path = "persistence-schema83-recruitment.ccsave";
    RemoveDatabase(path);
    CcSimInit(&legacy, 42U);
...
static void CheckSchema84RecruitmentJournal(void)
{
    static CcSim legacy, after, restored;
    char error[256];
    const char *path = "persistence-schema84-recruitment.ccsave";
    RemoveDatabase(path);
    CcSimInit(&legacy, 42U);
...
static void CheckSchema85RecruitmentJournal(void)
{
    static CcSim legacy, after, restored;
    char error[256];
    const char *path = "persistence-schema85-recruitment.ccsave";
    RemoveDatabase(path);
    CcSimInit(&legacy, 42U);
...
static void CheckSchema86RecruitmentJournal(void)
{
    static CcSim legacy, after, restored;
    char error[256];
    const char *path = "persistence-schema86-recruitment.ccsave";
    RemoveDatabase(path);
    CcSimInit(&legacy, 42U);
...
static void CheckSchema87RecruitmentJournal(void)
{
    static CcSim legacy, after, restored;
    char error[256];
    const char *path = "persistence-schema87-recruitment.ccsave";
    RemoveDatabase(path);
    CcSimInit(&legacy, 42U);
...
static void CheckSchema89RecruitmentJournal(void)
{
    static CcSim legacy, after, restored;
    char error[256];
    const char *path = "persistence-schema89-recruitment.ccsave";
    RemoveDatabase(path);
    CcSimInit(&legacy, 42U);
...
static void CheckSchema90RecruitmentJournal(void)
{
    static CcSim legacy, after, restored;
    char error[256];
    const char *path = "persistence-schema90-recruitment.ccsave";
    RemoveDatabase(path);
    CcSimInit(&legacy, 42U);
    legacy.schema_version = 90U;
    CC_CHECK(CcSimBeginArchiveRecruitment(&legacy));
    CcSimAdvanceDays(&legacy, 6);
    after = legacy;
    CcSimAdvanceDays(&after, 1);
    ...
    CC_CHECK(sqlite3_open(path, &database) == SQLITE_OK);
    sqlite3_close(database);
    AddLegacyDayJournalSuffix(path, &legacy, &after, 90U, 25U);
    CC_CHECK(CcSaveRead(path, &restored, error, sizeof(error)));
    CC_CHECK(restored.schema_version == CC_SIM_SCHEMA_VERSION);
    restored.schema_version = 90U;
    CC_CHECK(CcSimHash(&restored) == CcSimHash(&after));
    restored.schema_version = CC_SIM_SCHEMA_VERSION;
    CC_CHECK(CcSaveWrite(path, &restored, error, sizeof(error)));
    RemoveDatabase(path);
...
static void CheckSchema91RecruitmentJournal(void)
{
    static CcSim legacy, after, restored;
    char error[256];
    const char *path = "persistence-schema91-recruitment.ccsave";
    RemoveDatabase(path);
    CcSimInit(&legacy, 42U);
    legacy.schema_version = 91U;
    CC_CHECK(CcSimBeginArchiveRecruitment(&legacy));
    CcSimAdvanceDays(&legacy, 6);
    after = legacy;
    CcSimAdvanceDays(&after, 1);
    ...
    CC_CHECK(sqlite3_open(path, &database) == SQLITE_OK);
    sqlite3_close(database);
    AddLegacyDayJournalSuffix(path, &legacy, &after, 91U, 25U);
    CC_CHECK(CcSaveRead(path, &restored, error, sizeof(error)));
    CC_CHECK(restored.schema_version == CC_SIM_SCHEMA_VERSION);
    restored.schema_version = 91U;
    CC_CHECK(CcSimHash(&restored) == CcSimHash(&after));
    restored.schema_version = CC_SIM_SCHEMA_VERSION;
    CC_CHECK(CcSaveWrite(path, &restored, error, sizeof(error)));
    RemoveDatabase(path);
...
static void CheckSchema84HistoricalCast(void)
{
    static CcSim legacy, after, restored;
    char error[256];
    const char *path = "persistence-schema84-history.ccsave";
    RemoveDatabase(path);
...
int main(void)
{
    ...
    CheckSchema81RecruitmentJournal();
    CheckSchema82RecruitmentJournal();
    CheckSchema83RecruitmentJournal();
    CheckSchema84RecruitmentJournal();
    ...
    CheckSchema89RecruitmentJournal();
    CheckSchema90RecruitmentJournal();
    CheckSchema91RecruitmentJournal();
