...
{
    int32_t turns = 0;
    while (sim->journey.active && turns++ < 100000) {
...
static void CheckSchema6Compatibility(char *error, size_t error_capacity)
{
    ...
    /* Schema 6 predates treasures; this fixture only covers the older shape. */
    legacy.treasure_count = 0;
    CC_CHECK(CcSaveWrite(path, &legacy, error, error_capacity));
    CcSim restored;
    CC_CHECK(CcSaveRead(path, &restored, error, error_capacity));
    CC_CHECK(restored.schema_version == CC_SIM_SCHEMA_VERSION);
...
static void CheckJourneyStopPersistence(char *error, size_t error_capacity)
{
    ...
    CompleteJourney(&sim, error, error_capacity);
                                        ...
                                        size_t error_capacity)
{
    ...
    (void)snprintf(
        ...
        "%s/tests/fixtures/shipped/"
        "schema-104-generator-25-pilot-road-journal.ccsave",
        CC_TEST_SOURCE_DIR);
    /* Generated by the schema-104 runtime at d30028cc. The base hash and
       final action hash prove that the two journal rows remain intact. */
    CC_CHECK(ReadSqliteInteger(
        pilot_file,
        "SELECT schema_version=104 AND generator_version=25 AND "
        "state_hash='dfa5e2a1991660ea' FROM meta WHERE id=1;") == 1);
    CC_CHECK(ReadSqliteInteger(
        pilot_file, "SELECT COUNT(*) FROM action_journal;") == 2);
    CC_CHECK(ReadSqliteInteger(
        pilot_file,
        "SELECT post_state_hash='607d65c39be3cf66' FROM action_journal "
        "ORDER BY ordinal DESC LIMIT 1;") == 1);

    CcSim pilot;
    CC_CHECK(CcSaveRead(pilot_file, &pilot, error, error_capacity));
    CcPilotRoadTopology topology;
    CC_CHECK(pilot.schema_version == CC_SIM_SCHEMA_VERSION);
    CC_CHECK(pilot.generator_version == CC_GENERATOR_VERSION);
    CC_CHECK(CcPilotRoadTopologyBuild(&pilot, &topology));
    CC_CHECK(pilot.journey.active);
    CC_CHECK(pilot.journey.route_id == topology.route_id);
    CC_CHECK(pilot.journey.road_position_active);
    CC_CHECK(pilot.journey.road_compatibility_milli == 128);
    CC_CHECK(pilot.journey.road_coordinate_units ==
             CcRoadScaleDistance(topology.main_length_units, 128));
    CC_CHECK(pilot.journey.road_geometry_length_units ==
    ...
    CC_CHECK(CcTestBeforeCalendarHash(&pilot) == UINT64_C(4218671708288473085));
    ...
    CC_CHECK(CcTestBeforeCalendarHash(&stopped) == UINT64_C(11691673544305195830));
    ...
    CC_CHECK(CcTestBeforeCalendarHash(&checkpoint) == UINT64_C(13060270219596856870));
    ...
    CC_CHECK(CcTestBeforeCalendarHash(&mill_stop) == UINT64_C(10255154608615446321));

    char mine_file[512];
    (void)snprintf(
        mine_file, sizeof(mine_file),
        "%s/tests/fixtures/shipped/"
        "schema-104-generator-25-mine-road-journal.ccsave",
        CC_TEST_SOURCE_DIR);
    /* This fixture follows the real road and mine actions through the bypass
       to an active Lower Passage contest before schema-105 migration. */
    CC_CHECK(ReadSqliteInteger(
        mine_file,
        "SELECT schema_version=104 AND generator_version=25 AND "
        "state_hash='b661e90b3fe150f6' FROM meta WHERE id=1;") == 1);
    CC_CHECK(ReadSqliteInteger(
        mine_file, "SELECT COUNT(*) FROM action_journal;") == 54);
    CC_CHECK(ReadSqliteInteger(
        mine_file,
        "SELECT post_state_hash='e2d37241306e6abf' FROM action_journal "
        "ORDER BY ordinal DESC LIMIT 1;") == 1);

    CcSim mine;
    CC_CHECK(CcSaveRead(mine_file, &mine, error, error_capacity));
    CC_CHECK(mine.schema_version == CC_SIM_SCHEMA_VERSION);
    CC_CHECK(mine.generator_version == CC_GENERATOR_VERSION);
    CC_CHECK(mine.journey.active);
    CC_CHECK(!mine.journey.road_position_active);
    CC_CHECK(mine.mine.phase == CC_MINE_LEVEL);
    CC_CHECK(mine.mine.revision == 51);
    CC_CHECK(mine.mine.bypass_route_seen);
    CC_CHECK(mine.mine.contest_active);
    ...
    CC_CHECK(CcTestBeforeCalendarHash(&mine) == UINT64_C(12192256954273969565));
}
...
static void CheckSchema110RoadBlockJournalUpgrade(char *error,
                                                  size_t error_capacity)
{
    /* Created with the shipped schema-110 client, seed 0x205722. The
       closed road has a bandit claimant and one pending travel command.
       Checkpointed to SQLite DELETE mode for read-only fixture loading.
       SHA-256: 4e0273156867d4c8944f2c494202a9c161095ad3ff31ac5be34a80d50bddaaef */
    const char *fixture = CC_TEST_SOURCE_DIR
        "/tests/fixtures/shipped/"
        "schema-110-generator-25-road-block-journal.ccsave";
    CC_CHECK(ReadSqliteInteger(fixture,
        "SELECT schema_version=110 AND generator_version=25 AND "
        "journal_generation=1 AND journal_cursor=0 AND "
        "state_hash='2e7a035ead69ac16' FROM meta WHERE id=1;") == 1);
    CC_CHECK(ReadSqliteInteger(fixture,
        "SELECT COUNT(*) FROM action_journal WHERE ordinal=1 AND "
    ...
    CC_CHECK(CcTestBeforeCalendarHash(&restored) == UINT64_C(15091959056674726392));
...
{
    bool legacy = schema >= 2U && schema <= CC_SIM_NEWEST_LEGACY_SCHEMA;
    ...
    if (generator == CC_GENERATOR_VERSION) {
        ...
        if (schema >= 32U && schema <= CC_SIM_NEWEST_LEGACY_SCHEMA) return true;
    }
...
static void CheckSchema79RecruitmentJournal(void)
{
    ...
    sqlite3 *database = NULL;
    CC_CHECK(sqlite3_open(path, &database) == SQLITE_OK);
    ExecuteFixtureSql(database, "DROP TABLE archive_recruitment;", "drop recruitment fixture");
    sqlite3_close(database);
    AddLegacyDayJournalSuffix(path, &legacy, &after, 77U, 25U);
    CC_CHECK(CcSaveRead(path, &restored, error, sizeof(error)));
    CC_CHECK(restored.schema_version == CC_SIM_SCHEMA_VERSION);
...
static void CheckSchema84RecruitmentTrainingJournal(void)
{
    static CcSim legacy, after, restored;
    char error[256];
    const char *path = "persistence-schema84-recruitment-training.ccsave";
    RemoveDatabase(path);
    CcSimInit(&legacy, 42U);
    legacy.schema_version = 83U;
    CC_CHECK(CcSimBeginArchiveRecruitment(&legacy));
    CcSimAdvanceDays(&legacy, 6);
    after = legacy;
    CcSimAdvanceDays(&after, 1);
    ...
    sqlite3 *database = NULL;
    CC_CHECK(sqlite3_open(path, &database) == SQLITE_OK);
    ExecuteFixtureSql(database, "DROP TABLE archive_recruitment_training;", "drop recruitment fixture");
    sqlite3_close(database);
    AddLegacyDayJournalSuffix(path, &legacy, &after, 83U, 25U);
    CC_CHECK(CcSaveRead(path, &restored, error, sizeof(error)));
    CC_CHECK(restored.schema_version == CC_SIM_SCHEMA_VERSION);
...
static void CheckSchema85RecruitmentStaffJournal(void)
{
    static CcSim legacy, after, restored;
    char error[256];
    const char *path = "persistence-schema85-recruitment-staff.ccsave";
    RemoveDatabase(path);
    CcSimInit(&legacy, 42U);
    legacy.schema_version = 84U;
    CC_CHECK(CcSimBeginArchiveRecruitment(&legacy));
    CcSimAdvanceDays(&legacy, 6);
    after = legacy;
    CcSimAdvanceDays(&after, 1);
    ...
    sqlite3 *database = NULL;
    CC_CHECK(sqlite3_open(path, &database) == SQLITE_OK);
    ExecuteFixtureSql(database, "DROP TABLE archive_staff;", "drop recruitment fixture");
    sqlite3_close(database);
    AddLegacyDayJournalSuffix(path, &legacy, &after, 84U, 25U);
    CC_CHECK(CcSaveRead(path, &restored, error, sizeof(error)));
    CC_CHECK(restored.schema_version == CC_SIM_SCHEMA_VERSION);
...
static void CheckSchema86RecruitmentAutomaticJournal(void)
{
    static CcSim legacy, after, restored;
    char error[256];
    const char *path = "persistence-schema86-recruitment-automatic.ccsave";
    RemoveDatabase(path);
    CcSimInit(&legacy, 42U);
    legacy.schema_version = 85U;
    CC_CHECK(CcSimBeginArchiveRecruitment(&legacy));
    CcSimAdvanceDays(&legacy, 6);
    after = legacy;
    CcSimAdvanceDays(&after, 1);
    ...
    CC_CHECK(sqlite3_open(path, &database) == SQLITE_OK);
    sqlite3_close(database);
    AddLegacyDayJournalSuffix(path, &legacy, &after, 85U, 25U);
    CC_CHECK(CcSaveRead(path, &restored, error, sizeof(error)));
    CC_CHECK(restored.schema_version == CC_SIM_SCHEMA_VERSION);
...
static void CheckSchema87RecruitmentBindingJournal(void)
{
    static CcSim legacy, after, restored;
    char error[256];
    const char *path = "persistence-schema87-recruitment-binding.ccsave";
    RemoveDatabase(path);
    CcSimInit(&legacy, 42U);
    legacy.schema_version = 86U;
    CC_CHECK(CcSimBeginArchiveRecruitment(&legacy));
    CcSimAdvanceDays(&legacy, 6);
    after = legacy;
    CcSimAdvanceDays(&after, 1);
    ...
    CC_CHECK(sqlite3_open(path, &database) == SQLITE_OK);
    sqlite3_close(database);
    AddLegacyDayJournalSuffix(path, &legacy, &after, 86U, 25U);
    CC_CHECK(CcSaveRead(path, &restored, error, sizeof(error)));
    CC_CHECK(restored.schema_version == CC_SIM_SCHEMA_VERSION);
...
static void CheckSchema88RecruitmentVolumeJournal(void)
{
    static CcSim legacy, after, restored;
    char error[256];
    const char *path = "persistence-schema88-recruitment-volume.ccsave";
    RemoveDatabase(path);
    CcSimInit(&legacy, 42U);
    legacy.schema_version = 87U;
    CC_CHECK(CcSimBeginArchiveRecruitment(&legacy));
    CcSimAdvanceDays(&legacy, 6);
    after = legacy;
    CcSimAdvanceDays(&after, 1);
    ...
    CC_CHECK(sqlite3_open(path, &database) == SQLITE_OK);
    sqlite3_close(database);
    AddLegacyDayJournalSuffix(path, &legacy, &after, 87U, 25U);
    CC_CHECK(CcSaveRead(path, &restored, error, sizeof(error)));
    CC_CHECK(restored.schema_version == CC_SIM_SCHEMA_VERSION);
...
static void CheckSchema89RecruitmentReserveJournal(void)
{
    static CcSim legacy, after, restored;
    char error[256];
    const char *path = "persistence-schema89-recruitment-reserve.ccsave";
    RemoveDatabase(path);
    CcSimInit(&legacy, 42U);
...
static void CheckSchema89HistoricalCast(void)
{
    static CcSim legacy, after, restored;
    char error[256];
    const char *path = "persistence-schema89-history.ccsave";
    RemoveDatabase(path);
    CcSimInit(&legacy, UINT32_C(0x9e3779b9));
    legacy.schema_version = 88U;
    for (int day = 0; day < 36500; ++day) CcSimAdvanceDays(&legacy, 1);
    CC_CHECK(CcSimValidate(&legacy, error, sizeof(error)));
    CC_CHECK(CcSaveWrite(path, &legacy, error, sizeof(error)));
    after = legacy; CcSimAdvanceDays(&after, 1);
    AddLegacyDayJournalSuffix(path, &legacy, &after, 88U, 25U);
    CC_CHECK(CcSaveRead(path, &restored, error, sizeof(error)));
    restored.schema_version = 88U;
    CC_CHECK(CcSimHash(&restored) == CcSimHash(&after));
...
static void CheckSchema90RecruitmentLifetimeJournal(void)
{
    static CcSim legacy, after, restored;
    char error[256];
    const char *path = "persistence-schema90-recruitment-lifetime.ccsave";
    RemoveDatabase(path);
    CcSimInit(&legacy, 42U);
    legacy.schema_version = 89U;
    CC_CHECK(CcSimBeginArchiveRecruitment(&legacy));
    CcSimAdvanceDays(&legacy, 6);
    after = legacy;
    CcSimAdvanceDays(&after, 1);
    ...
    CC_CHECK(sqlite3_open(path, &database) == SQLITE_OK);
    sqlite3_close(database);
    AddLegacyDayJournalSuffix(path, &legacy, &after, 89U, 25U);
    CC_CHECK(CcSaveRead(path, &restored, error, sizeof(error)));
    CC_CHECK(restored.schema_version == CC_SIM_SCHEMA_VERSION);
...
static void CheckSchema95RecruitmentJournal(void)
{
    static CcSim legacy, after, restored;
    char error[256];
    const char *path = "persistence-schema95-recruitment.ccsave";
    RemoveDatabase(path);
    CcSimInit(&legacy, 42U);
    legacy.schema_version = 95U;
    CC_CHECK(CcSimBeginArchiveRecruitment(&legacy));
    CcSimAdvanceDays(&legacy, 6);
    after = legacy;
    CcSimAdvanceDays(&after, 1);
    ...
    CC_CHECK(sqlite3_open(path, &database) == SQLITE_OK);
    sqlite3_close(database);
    AddLegacyDayJournalSuffix(path, &legacy, &after, 95U, 25U);
    CC_CHECK(CcSaveRead(path, &restored, error, sizeof(error)));
    CC_CHECK(restored.schema_version == CC_SIM_SCHEMA_VERSION);
...
static void CheckRecentRecruitmentJournal(uint32_t schema)
{
    static CcSim legacy, after, restored;
    char error[256];
    const char *path = "persistence-recent-recruitment.ccsave";
    RemoveDatabase(path);
    CcSimInit(&legacy, 42U);
    ...
    CC_CHECK(sqlite3_open(path, &database) == SQLITE_OK);
    sqlite3_close(database);
    AddLegacyDayJournalSuffix(path, &legacy, &after, schema, 25U);
    CC_CHECK(CcSaveRead(path, &restored, error, sizeof(error)));
    CC_CHECK(restored.schema_version == CC_SIM_SCHEMA_VERSION);
...
int main(void)
{
    ...
    CheckSchema84RecruitmentTrainingJournal();
    CheckSchema85RecruitmentStaffJournal();
    CheckSchema86RecruitmentAutomaticJournal();
    CheckSchema87RecruitmentBindingJournal();
    CheckSchema88RecruitmentVolumeJournal();
    CheckSchema89RecruitmentReserveJournal();
