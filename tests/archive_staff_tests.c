...
int main(void)
{
    ...
    CC_CHECK(p.source_event_id != 0 && p.account_slot >= 0 && p.gate == CC_ARCHIVE_RECRUIT_MATERIALS);
    ...
    CC_CHECK(seat->stock[CC_GOOD_GOLD] == 0 && seat->stock[CC_GOOD_GEMS] == 0);
    RoundTrip();
    Prepared();
    CC_CHECK(CcSimAppointArchiveRecruit(&sim));
    ...
    seat = CcSimSettlementMutable(&sim, seat_id);
    seat->stock[CC_GOOD_GOLD] = seat->stock[CC_GOOD_GEMS] = 10;
