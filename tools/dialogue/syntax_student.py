...
def run(snapshot, model, probe, output, surface_probe=None, turns=8):
    validate_snapshot(snapshot)
    if type(turns) is not int or not 1 <= turns <= 32:
        raise ValueError('turns must be 1..32')
    output.mkdir(parents=True, exist_ok=False)
    history=[]; attempts=[]
    record={'format':FORMAT,'snapshot':snapshot,'attempts':attempts,'status':'running',
    ...
    try:
        for turn in range(turns):
            ...
            try:
                result=subprocess.run([str(probe),str(model),'--participant-prefix',text,'--generate'],
            ...
            act=validate(json.loads(result.stdout),person,heard)
            attempt['act']=act
...
def main():
    ...
    result=run(json.loads(args.snapshot.read_text()),args.model.resolve(),args.probe.resolve(),
               args.output,args.surface_probe.resolve() if args.surface_probe else None)
