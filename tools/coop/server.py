        ...
        for row in self.db.execute("SELECT id,state,view FROM worlds").fetchall():
            try:
                with self.engine.open(saved=row["state"]) as sim:
                    view = sim.snapshot()
                    if view == json.loads(row["view"]) and not sim.repaired:
                        continue
                    saved = sim.save()
