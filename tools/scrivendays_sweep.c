...
int main(int argc,char **argv)
{
    ...
    for(int seed=1;seed<=seeds;++seed) {
        ...
        if(valid) valid=CcSimValidate(&sim,error,sizeof(error));
        if(valid) {
            unsigned char *bytes=NULL;size_t length=0;
            valid=CcSaveEncode(&sim,&bytes,&length,error,sizeof(error)) &&
                CcSaveDecode(bytes,length,&reloaded,error,sizeof(error));
            CcSaveFreeBuffer(bytes);
            if(valid && CcSimHash(&sim)!=CcSimHash(&reloaded)) {
                valid=false;(void)snprintf(error,sizeof(error),"Save/reload hash differs.");
            }
        }
        int waiting=0;
