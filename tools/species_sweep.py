#!/usr/bin/env python3
"""Capture annual validation and sampled history for an six-species study."""
import argparse
...
def validate_rows(rows, seed, years):
    ...
    expected = [y for y in range(years + 1) if y <= 10 or y % 25 == 0 or y == years]
    if len(rows) != len(expected):
        raise ValueError(f'expected {len(expected)} sampled rows, received {len(rows)}')
    for year, row in zip(expected, rows):
...
def run_seed(binary, seed, years):
    ...
    command = [str(binary), '--seed', str(seed), '--years', str(years)]
    ...
    return dict(seed=seed, passed=passed, returncode=result.returncode,
                sampled_rows=len(rows), seconds=round(time.monotonic() - start, 3),
...
def main():
    ...
    manifest = dict(**fingerprint, binary=str(args.binary), jobs=args.jobs,
                    ...
                    command=[str(args.binary), '--seed', 'ORDINAL', '--years', str(args.years)],
                    sampling='Year 0, years 1 through 10, every 25 years, and final year. Daily observation and annual validation.',
